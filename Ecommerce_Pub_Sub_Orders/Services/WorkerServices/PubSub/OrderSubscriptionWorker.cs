...
{
    public class OrderSubscriptionWorker : IOrderSubscriptionWorker
    {
        private readonly ILogger<OrderSubscriptionWorker> _logger;
        private readonly StorageBroker _storageBroker;
        private readonly IPubSubBroker _pubSubBroker;

        public OrderSubscriptionWorker(
            StorageBroker storageBroker,
            IPubSubBroker pubSubBroker,
            ILogger<OrderSubscriptionWorker> logger
            )
        {
        ...
        {
            var processor = _pubSubBroker.OrderPersistanceProcessor();
        ...
        private async Task HandleMessageAsync(ProcessMessageEventArgs messageEventArgs)
        {
            try
            {
                _logger.LogInformation($"Mapping message to an order and adding to database, Date Time:{DateTime.UtcNow}");
                _storageBroker.Add(JsonConvert.DeserializeObject<Order>(messageEventArgs.Message.Body.ToString())!);
