...
{
    public class OrderDeliverySubscriptionWorker : IOrderDeliverySubscriptionWorker
    {
        private ILogger<OrderDeliverySubscriptionWorker> _logger;
        private readonly StorageBroker _storageBroker;
        private readonly IPubSubBroker _pubSubBroker;

        public OrderDeliverySubscriptionWorker(StorageBroker storageBroker, IPubSubBroker pubSubBroker)
        {
        ...
        {
            var processor = _pubSubBroker.OrderDeliveryProcessor();
        ...
        private async Task HandleMessageAsync(ProcessMessageEventArgs messageEventArgs)
        {
            try
            {
                _logger.LogInformation($"Mapping message to a delivery note and adding to database, Date Time:{DateTime.UtcNow}");
                Order order = JsonConvert.DeserializeObject<Order>(messageEventArgs.Message.Body.ToString())!;
                DeliveryNote deliveryNote = order.DeliveryDetails;
                _storageBroker.AddDeliveryNote(deliveryNote);
